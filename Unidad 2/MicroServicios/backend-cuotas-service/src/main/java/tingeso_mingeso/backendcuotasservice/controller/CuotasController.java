...
public class CuotasController {
    ...
    public ResponseEntity<CuotasEntity> pagarCuota(@PathVariable("idCuota") int idCuota) {
        Optional<CuotasEntity> cuotaOptional = cuotasService.findCuotaById(idCuota);

        if (cuotaOptional.isPresent()) {
            CuotasEntity cuota = cuotaOptional.get();
